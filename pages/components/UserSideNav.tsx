...
import { PanelLeftOpen, PanelRightClose, Workflow, Folders, NotebookTabs, Settings, Move3d, MoveHorizontal } from 'lucide-react';
            ...
            <div>
                ...
                <nav className={`side-navbar z-50 flex flex-col h-screen py-6 drop-shadow absolute top-0 left-0 transform ${isSidebarOpen ? 'translate-x-0 z-50' : '-translate-x-full'} transition-transform duration-300 h-screen ease-in-out z-50 ${className}`}>
                    ...
                    <ul className='flex flex-col flex-grow overflow-y-auto'>
                        <li className={`w-full flex justify-normal m-0 ${router.pathname == "companies/user/freight-rfq" ? "active" : ""}`}>
                            <Link href="companies/user/freight-rfq" className={`side-nav-btn text-stone-100 font-semibold w-full ${router.pathname == "companies/user/freight-rfq" ? "active" : ""}`}>
                        ...
                        <li className={`w-full flex justify-normal m-0 ${router.pathname == "companies/user/user-documents" ? "active" : ""}`}>
                            <Link href="companies/user/user-documents" className={`side-nav-btn text-stone-100 font-semibold w-full ${router.pathname == "companies/user/user-documents" ? "active" : ""}`}>
                        ...
                        <li className={`w-full flex justify-normal m-0 ${router.pathname == "companies/user/equipment-directory" ? "active" : ""}`}>
                            <Link href="companies/user/equipment-directory" className={`side-nav-btn text-stone-100 font-semibold w-full ${router.pathname == "companies/user/equipment-directory" ? "active" : ""}`}>
                    ...
                    <ul className='flex flex-col gap-4 justify-end items-center'>
                        <li className={`w-full text-nowrap flex justify-normal m-0 ${router.pathname == "companies/user/settings" ? "active" : ""}`}>
                            <Link href="companies/user/settings" className={`logout mt-4 md:mt-0 dark:bg-zinc-300 dark:text-zinc-700 flex items-center justify-center gap-2 font-semibold py-1 w-full ${router.pathname == "companies/user/settings" ? "active" : ""}`}>
                                <Settings />   Settings
                            </Link>
                        </li>
