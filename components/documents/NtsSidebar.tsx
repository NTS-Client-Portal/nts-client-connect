...
import { FolderHeart, Folder, X, FileText } from 'lucide-react';

const NtsSidebar = ({ sidebarOpen, setSidebarOpen, activeSection, setActiveSection }) => {
    return (
        <div className={`fixed inset-y-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out w-64 bg-zinc-200 dark:bg-zinc-900 dark:text-white p-4 border-r border-t border-zinc-700/20 shadow-lg z-50 md:relative md:translate-x-0 h-screen`}>
            ...
            <ul className="space-y-2">
                ...
                <li className='flex gap-1 items-center'>
                    <FileText />
                    <button
                        className={`w-full text-left p-2 ${activeSection === 'nts_documents' ? 'bg-zinc-100 dark:text-zinc-800' : ''}`}
                        onClick={() => setActiveSection('nts_documents')}
                    >
                        NTS Documents
                    </button>
                </li>
