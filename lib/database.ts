...
export async function fetchAllQuotes() {
	const { data, error } = await supabase.from("shippingquotes").select("*");
...
export async function addFreightItem(
	freight: Database["public"]["Tables"]["freight"]["Insert"]
): Promise<Database["public"]["Tables"]["freight"]["Row"] | null> {
	try {
		const { data, error } = await supabase
			.from("freight")
			.insert([freight])
...
export async function checkDuplicateInventoryNumber(
...
): Promise<Database["public"]["Tables"]["companies"]["Row"] | null> {
	try {
		const { data, error } = await supabase
			.from("companies")
			.insert([companies])
...
export async function fetchVendorsData() {
	const { data, error } = await supabase
		.from("vendors")
		.select(
			"id, vendornumber, vendorname, businessstreet, businesscity, businessstate, email, phone"
		);
...
export async function fetchPurchaseOrders(userId: string) {
	const { data, error } = await supabase
		.from("purchase_order")
		.select(
			"id, ponumber, status, createddate, expecteddate, vendornumber, vendorname, order_description, user_id"
		)
